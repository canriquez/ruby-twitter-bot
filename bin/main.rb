...
html_message, prompt = robot.build_mail(html_message, "checking last tweet update on #{TWEET_HASH} for handle : #{USER}")
puts prompt
...
if !last_on_twitter[0].nil?
  ...
  html_message, prompt = robot.build_mail(html_message, "failing to get a valid tweet publishd with #{TWEET_HASH} hash... for user #{USER}")
  puts prompt
  html_message, prompt = robot.build_mail(html_message, "building into report to #{USER} ")
  puts prompt
end
